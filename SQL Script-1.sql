...
SELECT customer.first_name, address.address, SUM(payment.amount) as SUMpay,
    SUM(SUM(payment.amount)) OVER (ORDER BY SUM(payment.amount) DESC) as total,
    ROUND(
        SUM(SUM(payment.amount)) OVER (ORDER BY SUM(payment.amount) DESC) * 100.0 / SUM(SUM(payment.amount)) OVER(), 1) || '%' as Prosent
FROM customer
JOIN payment ON customer.customer_id = payment.customer_id
JOIN address ON customer.address_id = address.address_id
GROUP BY customer.customer_id, customer.first_name, address.address_id, address.address
